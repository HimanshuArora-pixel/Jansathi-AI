    ...
    def __init__(self, collection_name="jansaathi_legal_kb"):
        ...
        if is_render:
            ...
            try:
                ...
                # Try offline first
                self.reranker = CrossEncoder('cross-encoder/ms-marco-MiniLM-L-12-v2', max_length=512, local_files_only=True, device='cpu')
            except Exception:
                try:
                    # If not cached, try downloading normally
                    print("Local cross-encoder not found. Attempting to download from Hugging Face...")
                    self.reranker = CrossEncoder('cross-encoder/ms-marco-MiniLM-L-12-v2', max_length=512, device='cpu')
