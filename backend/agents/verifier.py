...
def verifier_node(state: AgentState) -> dict:
    ...
    # to prevent forcing deadlines/roadmaps/citations onto general non-legal informational queries.
    # Note: General_Legal_Advice is intentionally NOT in skip_intents so that legal queries and any
    # misclassified advice still undergo quality, citation, and procedural verification.
    skip_intents = {"Civic_Scheme_Info", "Chitchat", "Off_Topic"}
    ...
    try:
        ...
        print(f"[Verifier] Running single-shot verification...")
        ...
        if not json_match:
            print(f"[Verifier] Failed to parse JSON, accepting current draft.")
        else:
            ...
            if not passes or score < 7.0:
                if issues:
                    print(f"[Verifier] Score {score} < 7.0 — triggering ONE-TIME re-draft...")
                    ...
                    current_response += "\n\n> ⚠️ **Verification Note:** This response was automatically corrected for compliance by the reflexion layer. Please verify key details independently."
                    has_modified = True
                else:
                    print(f"[Verifier] Score is low but no issues listed. Accepting draft.")
            else:
                print(f"[Verifier] Response accepted (score={score}/10).")
