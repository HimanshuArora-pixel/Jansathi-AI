...
def extract_claim_amount(message: str) -> float:
    ...
    message = message.translate(hindi_to_eng)
    
    # Handle ranges like "5-6 lakhs" or "10 to 20L" - take the upper bound
    range_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:-|to)\s*(\d+(?:\.\d+)?)\s*(lakh|lac|L\b|crore|cr|k\b|thousand)', message, re.IGNORECASE)
    if range_match:
        val = float(range_match.group(2))
        unit = range_match.group(3).lower()
        if unit in ('crore', 'cr'): return val * 100
        elif unit in ('lakh', 'lac', 'l'): return val
        elif unit in ('k', 'thousand'): return val / 100
        return val
    ...
    k_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:k\b|thousand)', message, re.IGNORECASE)
    if k_match:
        return float(k_match.group(1)) / 100
...
def extract_state(message: str) -> str:
    """Extracts Indian state name from user message (includes city mapping)."""
    states = {
        "maharashtra": ["maharashtra", "mumbai", "pune", "nagpur", "thane"],
        "karnataka": ["karnataka", "bengaluru", "bangalore", "mysore"],
        "delhi": ["delhi", "new delhi", "ncr", "gurgaon", "noida", "ghaziabad", "faridabad"],
        "uttar pradesh": ["up", "uttar pradesh", "lucknow", "kanpur", "agra", "varanasi"],
        ...
        "madhya pradesh": ["madhya pradesh", "mp", "indore", "bhopal"],
        "bihar": ["bihar", "patna"],
        "punjab": ["punjab", "ludhiana", "amritsar", "chandigarh"],
        "haryana": ["haryana", "panipat", "rohtak"],
    ...
    for state, keywords in states.items():
        if any(kw in msg_lower for kw in keywords):
            return state.title() if state != "up" and state != "mp" else state.upper()
    return ""
