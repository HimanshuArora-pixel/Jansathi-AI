...
const MD_COMPONENTS = {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  document: ({node, ...props}: any) => (
    <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="bg-[var(--color-bg-surface)] border border-[var(--color-border-dim)] p-6 rounded-lg my-6 font-serif shadow-sm text-[14px]" {...props} />
  ),
};
                  ...
                  <div className="flex items-start gap-3 w-full pl-4 border-l-2 border-[var(--color-border-accent)]">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        {msg.intent && !["chitchat", "off_topic", "off-topic", "general", "general_chat", "unknown"].includes(msg.intent.toLowerCase()) && (
