...
2. They have zero awareness of India's jurisdiction hierarchy (which forum to file in depends on claim amount, state, and case type â€” a generic LLM has no mechanism to compute this).
3. They cannot perform multi-step agentic workflows: classify intent, retrieve verified law, draft a formal document, self-verify the output, and persist it â€” all in one pipeline.
...
graph TD
    ...
    E --> H["Verifier Agent\n(Reflexion â€” score less than 7.5 triggers re-draft)"]
    F --> H
...
- **Model Performance:** Achieved a **Macro F1 Score of 94.8%** on the validation set. Per-epoch scores: Epoch 1 â†’ 92.7%, Epoch 2 â†’ 93.5%, Epoch 3 â†’ **94.8%**.
...
| Layer 3 | Forums | `DISTRICT_CONSUMER_COMMISSION` (&lt;â‚¹50L), `STATE_CONSUMER_COMMISSION` (â‚¹50Lâ€“â‚¹2Cr), `NATIONAL_CONSUMER_COMMISSION` (&gt;â‚¹2Cr), `RERA_AUTHORITY`, `POLICE_FIR` |
...
The key function `get_context_for_intent()` traverses this graph and injects verified facts as a `=== VERIFIED LEGAL FACTS (DO NOT contradict) ===` block into the LLM prompt â€” acting as a hard constraint against hallucination.
...
- **Filing fees** (â‚¹0 for claims &lt;â‚¹5 lakh, â‚¹200 for â‚¹5Lâ€“â‚¹10L, etc.)
...
Output is injected as `=== JURISDICTION ENGINE OUTPUT (Deterministic â€” Ground Truth) ===` alongside Knowledge Graph context. The LLM is instructed never to contradict these verified facts.
...
**Stage 1 â€” Query Expansion:** Fast LLM generates 3 alternate phrasings using formal Indian legal terminology. All 4 queries searched in parallel.

**Stage 2 â€” Dual Search:**
...
**Stage 3 â€” Reciprocal Rank Fusion (RRF):** Vector and BM25 results merged with configurable weights (0.6 vector, 0.4 BM25). Formula: `score = weight * (1 / (60 + rank))`.

**Stage 4 â€” Cross-Encoder Reranking:** Merged candidates re-scored by `cross-encoder/ms-marco-MiniLM-L-12-v2` (33M params). Top-3 results by semantic relevance returned.
...
**`draft_document_node()`** â€” For drafting intents (RTI, FIR, Legal Notice, etc.):
...
**`legal_advice_node()`** â€” For advice intents:
- Generates structured advice with âš–ï¸ Your Legal Rights, ðŸ—ºï¸ Action Roadmap, ðŸ“ž Key Contacts sections.
- Falls back to conversational reply for specific follow-up questions.

**`general_chat_node()`** â€” For chitchat and civic info queries.

---

### 6. Verifier Agent â€” Reflexion Loop (`agents/verifier.py`)
...
- `detect_language()` â€” detects Hindi (Devanagari Unicode block >10%), Hinglish (keyword matching, threshold â‰¥ 2 words), or English.
- `get_language_instruction()` â€” returns a `LANGUAGE RULE` string injected into every LLM system prompt, forcing reply language to match user's input.
...
2. **OCR.Space API fallback** â€” If PDF is scanned/image-based, converts first page to PNG and calls OCR.Space API. No heavy Tesseract dependency needed on the server.
...
|---------|-----------------|-----------|
| Hallucinated law sections | âŒ Common | âœ… Blocked by Knowledge Graph hard constraints |
| Wrong court / jurisdiction | âŒ Common | âœ… Deterministic rules engine computes exact forum |
| Reply in user's language | âŒ Ignores | âœ… Auto-detects Hindi/Hinglish/English, replies to match |
| Self-correction | âŒ None | âœ… Reflexion loop scores every response, re-drafts if &lt;7.5/10 |
| Domain restriction | âŒ Answers anything | âœ… Hard guardrail blocks off-topic, fiction, recipes |
| Intent specialization | âŒ Generic | âœ… 19 specialized classes, 94.8% accuracy |
| Document persistence | âŒ None | âœ… Drafted docs saved to DB, downloadable as PDF |
| Scanned PDF support | âŒ None | âœ… OCR.Space API fallback for image-based forms |
...
â”œâ”€â”€ backend/
â”‚   â”œâ”€â”€ agents/
â”‚   â”‚   â”œâ”€â”€ graph.py              # LangGraph StateGraph â€” all nodes, edges, conditional routing
â”‚   â”‚   â”œâ”€â”€ state.py              # AgentState TypedDict (messages, intent, context, score, etc.)
â”‚   â”‚   â”œâ”€â”€ intent_router.py      # 19-class intent classification (local model + Groq fallback)
â”‚   â”‚   â”œâ”€â”€ graph_lookup.py       # Knowledge Graph + Jurisdiction Engine injection node
â”‚   â”‚   â”œâ”€â”€ retriever.py          # Hybrid RAG retrieval node
â”‚   â”‚   â”œâ”€â”€ drafter.py            # Draft, Advice, and General Chat nodes
â”‚   â”‚   â”œâ”€â”€ verifier.py           # Reflexion self-correction node (8-criteria critic)
â”‚   â”‚   â””â”€â”€ analyzer.py           # PDF contract analysis agent
â”‚   â”œâ”€â”€ knowledge/
â”‚   â”‚   â”œâ”€â”€ legal_graph.py        # NetworkX DiGraph (441 lines, 4-layer legal ontology)
â”‚   â”‚   â””â”€â”€ jurisdiction_engine.py # Deterministic rules engine (310 lines, 0 LLM calls)
â”‚   â”œâ”€â”€ rag/
â”‚   â”‚   â”œâ”€â”€ chroma_store.py       # ChromaDB + BGE embeddings
â”‚   â”‚   â””â”€â”€ pipeline.py           # 4-stage hybrid RAG (expand â†’ BM25+vector â†’ RRF â†’ rerank)
â”‚   â”œâ”€â”€ guardrails/
â”‚   â”‚   â””â”€â”€ schemas.py            # Pydantic models for structured legal outputs
â”‚   â”œâ”€â”€ templates/                # Jinja2 templates for RTI, Legal Notice, Consumer, RERA PDFs
â”‚   â”œâ”€â”€ utils/
â”‚   â”‚   â”œâ”€â”€ llm_utils.py          # think-tag stripping for reasoning models
â”‚   â”‚   â”œâ”€â”€ language_utils.py     # Hindi/Hinglish/English detection + LLM instruction injection
â”‚   â”‚   â”œâ”€â”€ placeholder_utils.py  # Extracts [PLACEHOLDER] fields from drafted documents
â”‚   â”‚   â””â”€â”€ template_renderer.py  # Jinja2 to HTML to PDF (WeasyPrint)
â”‚   â”œâ”€â”€ auth/                     # JWT auth, bcrypt, SQLAlchemy models
â”‚   â”œâ”€â”€ api/
â”‚   â”‚   â”œâ”€â”€ chat_router.py        # /api/chat/ endpoints (send, list, delete conversations)
â”‚   â”‚   â””â”€â”€ documents.py          # /api/documents/ endpoints (CRUD + PDF analysis + OCR)
â”‚   â”œâ”€â”€ training/
â”‚   â”‚   â”œâ”€â”€ retrain_classifier_v2.py  # Fine-tuning script (3 epochs, 94.8% F1, 19 classes)
â”‚   â”‚   â””â”€â”€ ingest_knowledge_base.py  # ChromaDB ingestion script
â”‚   â”œâ”€â”€ data/
â”‚   â”‚   â””â”€â”€ datasets/
â”‚   â”‚       â”œâ”€â”€ intent_training.jsonl    # Original 6-class dataset (~1,500 examples)
â”‚   â”‚       â””â”€â”€ intent_training_v2.jsonl # 19-class dataset (4,650 examples)
â”‚   â””â”€â”€ models/
â”‚       â””â”€â”€ intent_classifier/    # Fine-tuned InLegalBERT (438MB, 94.8% F1, 19 classes)
â”œâ”€â”€ frontend/
â”‚   â””â”€â”€ src/app/
â”‚       â”œâ”€â”€ login/page.tsx        # Auth UI (login + register)
â”‚       â”œâ”€â”€ chat/page.tsx         # Main chat interface with sidebar
â”‚       â””â”€â”€ dashboard/page.tsx    # My Documents dashboard with PDF export
â”œâ”€â”€ setup.ps1                     # One-command automated setup script
